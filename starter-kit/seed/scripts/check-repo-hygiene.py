...
APPROVED_ROOT_FILES = {
    "README.md", "AGENTS.md", "SOUL.md", "TOOLS.md", "LESSONS.md", "CLAUDE.md",
...
def check_artifacts(violations: list[str]) -> None:
    ...
    for entry in artifacts.iterdir():
        ...
        for child in entry.iterdir():
            if child.is_file():
                violations.append(f"loose file under artifacts/{entry.name}/: {child.name}")
                continue
            if not BUNDLE_DIR_RE.match(child.name):
                violations.append(
                    f"artifact bundle not dated/kebab-case: artifacts/{entry.name}/{child.name}"
                )
            has_report = (child / "REPORT.md").exists() or (child / "VERDICT.md").exists()
            has_index = (child / "index.md").exists() or (child / "artifacts.json").exists()
            if not has_report:
                violations.append(f"bundle missing REPORT.md/VERDICT.md: {child.relative_to(REPO_ROOT)}")
            if not has_index:
                violations.append(f"bundle missing index.md/artifacts.json: {child.relative_to(REPO_ROOT)}")
