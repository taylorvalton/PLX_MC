...
| Vercel (staging hosting, `petralabx/plx-mission-control`) | Active (provisioned 2026-06-11; staging domain `mc-staging.plxcustomer.io`, prod alias `plx-mission-control.vercel.app`) | Owner: Vince. Scope: runtime (staging). Auth: `VERCEL_API_TOKEN` from secrets manager; pushes to `main` auto-deploy via the GitHub connection. Default: generated URLs gated by Vercel SSO; the custom domain is gated by the Basic-auth staging middleware (`src/middleware.ts`, secret `PLX_MC_STAGING_PASSWORD` — gate is dormant when unset, so local dev is open). Kill switch: unset the Vercel domain or pause the project; sync scheduler stays OFF on Vercel (`PLX_MC_SYNC_ENABLED=0` — serverless timers are unreliable; sweeps are manual or move to Vercel Cron later). Health: Vercel deploy status + `/api/state` probe. Fallback: the dev box's Tailscale URL. Audit boundary: same `plx_mc` staging database and `sync_audit_log` as local. |
