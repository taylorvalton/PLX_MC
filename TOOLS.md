...
| PLX-MC MCP (task lifecycle + swarm compose) | Default-OFF (shipped 2026-06-24) | Owner: Vince. Scope: split — HTTPS cursor API (`/api/cursor/*`) and Streamable HTTP (`/api/cursor/mcp`) ship with the Vercel web app at `https://mc.plxcustomer.io`; **stdio client** (`tools/plx-mc-mcp/`) and **agentic swarm** (loopback `127.0.0.1:8900`) are **operator-local only** and are not part of the Vercel deploy. Auth: `PLX_MC_MCP_API_KEY` + allowlisted `X-MC-Operator-Email` (Secrets Manager); swarm via `SWARM_KEY_CMD`. Default: OFF — `PLX_MC_MCP_ENABLED=0`. Kill switch: same flag or remove MCP server from config; `SWARM_DISPATCH_ENABLED=0` disables dispatch tools only. Health: `mc_self_check`. Fallback: `scripts/compliance-checkout.mjs`, `/api/compliance/*`, `bin/swarm ask`. Audit: `mcp.tool.invoked` in `mc_events`. Module: `docs/modules/mcp/README.md`. |
...
| Vercel (production hosting, project `plx-mission-control`) | Active (provisioned 2026-06-11; **production** public URL `https://mc.plxcustomer.io`, staging fallback `mc-staging.plxcustomer.io`, Vercel alias `plx-mission-control.vercel.app`) | Owner: Vince. Scope: runtime — production web app + HTTPS APIs on Vercel project `plx-mission-control` (GitHub repo `petralabx/plx-mission-control`). Auth: `VERCEL_API_TOKEN` from secrets manager; pushes to `main` auto-deploy via the GitHub connection. Default: generated URLs gated by Vercel SSO; the custom domain is gated by app middleware (`src/middleware.ts`) — Entra OIDC when `PLX_MC_AUTH_*` is configured, Basic-auth break-glass (`PLX_MC_STAGING_PASSWORD`) otherwise, dormant locally. Kill switch: unset the Vercel domain or pause the project; the in-app `setInterval` scheduler stays OFF on Vercel (`PLX_MC_SYNC_ENABLED=0` — serverless timers are unreliable), so the recurring 5-min **delta sweep** runs via **Vercel Cron** (`vercel.json` → `GET /api/cron/sweep`, authed by the Vercel-injected `CRON_SECRET` bearer); manual sweeps remain available via `POST /api/sync/sweep`. Kill switch for the scheduled sweep: remove the `crons` entry in `vercel.json` or unset `CRON_SECRET` (the route then returns 503). Health: Vercel deploy status + `/api/state` probe. Fallback: the dev box's Tailscale URL. Audit boundary: `plx_mc` database and `sync_audit_log`. Evidence: root `README.md` (`Public app: https://mc.plxcustomer.io`); cutover redeploy aliases in `artifacts/sync/2026-07-13-prod-site-cutover/`. Operator-local MCP stdio + swarm are **not** hosted here. |
